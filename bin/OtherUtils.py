...
def SaveResult(Data, FileKey, OverWrite=False, FileCols=None, Recycle='D'):    
    ...
    if(os.path.isfile(FileName) and (not OverWrite)):
        ...
        if(Recycle == 'W'): Data.to_csv(gv.RptDir+gv.RepFiles[FileKey], mode='a', header=False)
    else: 
        Data.to_csv(FileName) 
        if(Recycle == 'W'): Data.to_csv(gv.RptDir+gv.RepFiles[FileKey])
    return()
...
def GetStockList(Stocks='', FileName=[]):
    if(Stocks == ''):
...
def LogTime():
    return(ExchTime().strftime('%d %H:%M:%S'))
def MyPrint(*args, **kwargs):
    if(gv.LogFileEnabled):
        FN=open(gv.LogFile,'a')
        print(*args, file=FN, **kwargs)
        FN.close()
    print(*args, **kwargs); 
    return()
def ExchTime(ExchZone=gv.NSEZone):
    utc = dt.datetime.utcnow(); utc = utc.replace(tzinfo=tz.gettz('UTC'))
    return(utc.astimezone(ExchZone).replace(tzinfo=None).replace(microsecond=0))